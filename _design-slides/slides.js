...
(function () {
  ...
  window.__palestra = {
    ...
    /* A nota exportada carrega TUDO o que o formador precisa no palco: o tempo
       acumulado, a fala e o aviso. Antes só o corpo ia, e a transição escrita
       no .notas-aviso se perdia justamente no arquivo que se usa ao vivo.
       O innerText traz a indentação do HTML, então normaliza o espaço. */
    notas: () => slides.map(s => {
      const limpar = (el) => el ? el.innerText.replace(/\s*\n\s*/g, '\n').replace(/[ \t]{2,}/g, ' ').trim() : '';
      const tempo = limpar(s.querySelector('.notas-tempo'));
      const corpo = limpar(s.querySelector('.notas-corpo'));
      const aviso = limpar(s.querySelector('.notas-aviso'));
      return [tempo && '[' + tempo + ']', corpo, aviso && '⚠ ' + aviso]
        .filter(Boolean).join('\n\n');
    })
  };
})();
